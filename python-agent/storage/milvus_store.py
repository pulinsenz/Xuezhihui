"""
Milvus 向量库实现（生产环境使用）。
启动 Milvus 后配置 MILVUS_URI，将 VECTOR_STORE 切换为 milvus 即可。
"""
import os
...
COLLECTION_NAME = os.getenv("MILVUS_COLLECTION", "xuezhihui_docs")


class MilvusStore(VectorStore):
    """基于 pymilvus MilvusClient 的向量库实现"""

    def __init__(self, embedding, uri: str = None):
    ...
    def _ensure_collection(self):
        ...
        agent_event(logger, "milvus_collection_created", collection=COLLECTION_NAME)
    ...
    def delete_knowledge(self, knowledge_id):
        self.client.delete(COLLECTION_NAME, filter=f'knowledge_id == "{knowledge_id}"')
    ...
    def search(self, query, knowledge_id=None, top_k=5):
        ...
        results = self.client.search(
            COLLECTION_NAME, data=[q_vec], limit=top_k, output_fields=["text"],
