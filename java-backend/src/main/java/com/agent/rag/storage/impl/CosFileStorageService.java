...
/**
 * 腾讯云 COS 文件存储实现（封面等公网图片）
 * <p>
 * store() 委托本地存储（文档仍走共享卷，python worker 按绝对路径读取）；
 * 仅 storeForWeb()（封面）上传 COS：桶为私有时给对象单独加 public-read，
...
public class CosFileStorageService implements FileStorageService {
    ...
    public String storeForWeb(MultipartFile file, Long userId) {
        ...
        try (InputStream in = file.getInputStream()) {
            ...
            log.error("封面上传 COS 失败: {}", originalName, e);
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "封面上传失败");
        }
        String url = host + "/" + key;
        log.info("封面已上传 COS: key={}, url={}", key, url);
        return url;
    }
